...
static int avDisconnectSinkFullPath(DBusConnection *conn, const char *fullpath)
{
	dbus_bool_t success;
	DBusMessage *msg;
        
        printf("fullpath is %s\n",fullpath);
	msg = dbus_message_new_method_call("org.bluez", fullpath,
						"org.bluez.AudioSink",
						"Disconnect");
...
static int avconnectSinkFullPath(DBusConnection *conn, const char *fullpath)
{
	dbus_bool_t success;
	DBusMessage *msg;
...
void ShowSignalStrength(short strength)
{
short pos_str;
...
void ClearScannedDeviceList(void)
{
 int i;
   for (i=0;i<15;i++)
	{
...
int GetAdapters(DBusConnection *conn)
{
...
static int StopDiscovery(DBusConnection *conn, const char *adapter_path,
						const char *agent_path,
						const char *device)
{
	dbus_bool_t success;
...
static int StartDiscovery(DBusConnection *conn, const char *adapter_path,
						const char *agent_path,
						const char *device)
{
	dbus_bool_t success;
...
static void parse_device(DBusMessage *msg)
{
...
if (StartDiscovery(conn, adapter_path, agent_path,
						 dbus_device) < 0) {
...
printf("now stopping...\n");
      if (StopDiscovery(conn, adapter_path, agent_path,
						 dbus_device) < 0) {
			dbus_connection_unref(conn);
			exit(1);
		       }
...
BT_error BT_ListKnownDevices(tGetAdapter *p_get_adapter)
{
     DBusError e;
DBusMessageIter arg_i, element_i, variant_i;
char **paths = NULL;
...
BT_error BT_PairDevice(tScannedDevices *p_scanned_device)
{
      BT_LOG(("BT_PairDevice\n"));
              ...
              if (create_paired_device(conn, adapter_path, agent_path,
						capabilities, p_scanned_device->bd_address) < 0) {
			dbus_connection_unref(conn);
